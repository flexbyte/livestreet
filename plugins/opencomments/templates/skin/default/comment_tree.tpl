...
{else}
	...
	{else}
		<div class="reply-area">
			<h4 class="reply-header" id="add_comment_root"><a href="#" onclick="document.commentCaptcha.src='{cfg name='path.root.engine_lib'}/external/kcaptcha/index.php?{$_sPhpSessionName}={$_sPhpSessionId}&n='+Math.random(); ls.comments.toggleCommentForm(0); return false;">{$sNoticeCommentAdd}</a></h4>	
			<div id="reply_0" class="reply">
				<div class="comment-preview" id="comment_preview_0" style="display: none;"><div class="comment-inner"><div class="content"></div></div></div>
				
				<form action="" method="POST" id="form_comment" onsubmit="return false;" enctype="multipart/form-data">
					
          <div style="padding-top:15px; padding-bottom:5px;"><input type="text" name="guest_name" value="" style="width:200px;" /> {$aLang.opencomments_name}</div>
          {if $oConfig->GetValue('opencomments.ask_mail')}
          <div style="padding-bottom:10px;"><input type="text" name="guest_email" value="" style="width:200px;" /> {$aLang.opencomments_mail}</div>
          {/if}
          <span>{$aLang.opencomments_title}</span><br />
